        ...
        try:
            ...
            if ret == RET_OK:
                ...
                if len(data) > 0:
                    ...
                    print(f"币种:         {row.get('currency', 'HKD')}")
                    print(f"总资产:       HK${row.get('total_assets', 0):,.2f}")
                    print(f"现金:         HK${row.get('cash', 0):,.2f}")
                    print(f"证券市值:     HK${row.get('market_val', 0):,.2f}")
                    print(f"可用资金:     HK${row.get('avl_withdrawal_cash', 0):,.2f}")
                    print(f"购买力:       HK${row.get('power', 0):,.2f}")
                    print(f"最大购买力:   HK${row.get('max_power_short', 0):,.2f}")
                print(f"{'='*70}\n")
                return data
            else:
    ...
    def buy(self, stock_code, price, qty, order_type=OrderType.NORMAL):
        ...
        try:
            ...
            if ret == RET_OK:
                ...
                print(f"3. 股票代码错误")
                print(f"4. 交易时间限制")
                print(f"5. 数量不符合规则（100股起，100的整数倍）")
                print()
                return None
    ...
    def get_market_snapshot(self, stock_code):
        ...
        try:
            ...
            if ret == RET_OK:
                ...
                print(f"{'='*70}")
                print(f"最新价:   HK${snapshot['last_price']:.3f}")
                print(f"开盘价:   HK${snapshot['open_price']:.3f}")
                print(f"最高价:   HK${snapshot['high_price']:.3f}")
                print(f"最低价:   HK${snapshot['low_price']:.3f}")
                print(f"昨收价:   HK${snapshot['prev_close_price']:.3f}")
                print(f"成交量:   {snapshot['volume']:,.0f}")
                print(f"成交额:   HK${snapshot['turnover']:,.2f}")
                print(f"涨跌额:   HK${snapshot['change_val']:.3f}")
                print(f"涨跌幅:   {snapshot['change_rate']:.2f}%")
                print(f"换手率:   {snapshot.get('turnover_rate', 0):.2f}%")
                print(f"{'='*70}\n")
                return snapshot
            else:
...
if __name__ == '__main__':
    print("\n" + "="*70)
    print("Futu 港股交易测试 - 东方甄选 (HK.01797)")
    print("="*70)
    ...
    print("="*70)
    print("\n示例：买入500股东方甄选")
    print("  current_price = trader.get_current_price('HK.01797')")
    print("  trader.buy('HK.01797', current_price, 100)")
    print()
    print("示例：市价买入500股东方甄选")
    print("  trader.buy('HK.01797', 0, 200, order_type=OrderType.MARKET)")
    print()
    
    # 取消注释以测试
    # current_price = trader.get_current_price('HK.01797')
    # if current_price:
    #     trader.buy('HK.01797', current_price, 500)
